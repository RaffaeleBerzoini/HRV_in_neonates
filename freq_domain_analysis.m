function [LF2HF_welch, LF2HF_YW]=freq_domain_analysis(RRintervals, r_peaks_pt, f, fs) % o r_peaks_fp
...
subplot(1,3,2); plot(f,PSD_welch);
title('PSD_Welch RR_ECG','Interpreter','none'); xlabel('Frequency [Hz]'); ylabel('PSD Welch Method [ms^2/Hz]');
str = ['LF/HF=',num2str(LF2HF_welch)]; text(0.6,0.8*max(PSD_welch),str,'HorizontalAlignment','left');

subplot(1,3,3); plot(f,PSD_YW);
title('PSD_YW RR_ECG','Interpreter','none'); xlabel('Frequency [Hz]'); ylabel('PSD Yule-Walker Method [ms^2/Hz]');
